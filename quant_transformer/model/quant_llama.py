...
from quant_transformer.quantization.migration_llama import migration
...
class QuantizedLlamaAttention(QuantizedModule):
    ...
    def forward(
        ...
        query_states = self.query_permute_post_act_fake_quant(query_states, observation_mask, 2)
        key_states = self.key_transpose_post_act_fake_quant(key_states, observation_mask, 2)
        value_states = self.value_permute_post_act_fake_quant(value_states, observation_mask, 2)
...
class QuantizedLlamaDecoderLayer(QuantizedModule):
    ...
    def __init__(self, org_module, w_qconfig, a_qconfig, qinput=True, backend='academic'):
        ...
        self.second_layer_norm = QuantizedLlamaRMSNorm(org_module.post_attention_layernorm)
        ...
        self.gate_proj = QuantizedLayer(org_module.mlp.gate_proj, None, w_qconfig, a_qconfig, False)
        self.up_proj = QuantizedLayer(org_module.mlp.up_proj, None, w_qconfig, a_qconfig, False)
        if getattr(self.a_qconfig, 'disable_down_proj', False):
            ...
            self.down_proj = QuantizedLayer(org_module.mlp.down_proj, None, w_qconfig, a_qconfig, True)
    ...
    def forward(
        ...
        if self.cac_migrate:
            ...
            # calculate scale
            weight_list = torch.cat([self.gate_proj.module.weight,
                                     self.up_proj.module.weight])
            extra_dict = {
                'observation_mask': observation_mask,
                'act_fn': self.act_fn
            }
            best_scale = \
                migration(hidden_states, weight_list, self.a_qconfig, self.w_qconfig, 'up_and_gate', extra_dict)
            # update scale
            hidden_states /= best_scale
        ...
        hidden_states = self.act_fn(self.gate_proj(hidden_states, observation_mask, 1)) * self.up_proj(hidden_states, observation_mask, 1)
        ...
        if not self.a_qconfig.disable_down_proj and self.cac_migrate:
            ...
            best_scale = \
                migration(hidden_states, weight_list, self.a_qconfig, self.w_qconfig, 'down_proj', extra_dict)
            # update scale
            hidden_states /= best_scale
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        ...
                        return module(*inputs, output_attentions, None, observation_mask)
...
class QuantizedLlamaForCausalLM(PreTrainedModel, QuantizedModule):
    ...
    def __init__(self, org_module, w_qconfig, a_qconfig, qinput=True, backend='academic', is_remove_padding=False):
        ...
        self._no_split_modules = ["QuantizedLlamaDecoderLayer", "QuantizedLayer", "QuantizedModule"]
        self.qinput = qinput
        self.config = org_module.config
