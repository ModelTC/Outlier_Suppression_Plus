...
import logging
from quant_transformer.quantization.fake_quant import QuantizeBase
...
def token_wise_clipping(model, fp_input, fp_output, config, batch_size):
    ...
    if hasattr(config_quant.a_qconfig, 'token_quantile'):
        ...
        calibrate(model, fp_input)
        logger.info('the best percentile is {}'.format(config_quant.a_qconfig.token_quantile))
    else:
        step, iters = cac_step_iters(config_quant.a_qconfig.bit, batch_size)
        find_ratio(model, fp_input, fp_output,
